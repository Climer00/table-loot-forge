...
function categoryOf(type,rarity,attune){
  let cat;
  ...
  let line=cat+", "+String(rarity).toLowerCase();
  if(attune)line+=" (requires attunement)";
  return line;
}
...
function generateOnce(rarity,type){
  ...
  if(type==="Scroll")item=scrollItem(rarity,s);
    ...
    item={name:nameOf(type),slot:SL[type],attune:!!g.attune,properties:g.properties||[]};
  }
...
function ensureCategory(item){
  ...
  return categoryOf(item.type,item.rarity,item.attune);
}
...
function cardHtml(item,opts){
  ...
  if(item.attune)chips.push('<span class="chip attune">Attunement</span>');
