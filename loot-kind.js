...
(function(g){
  ...
  var WOND_FIX={"Cowl-helm":"helm","Hood-cap":"cap","Half-cape":"cape","Browguard":"helm","Peakguard":"helm","Wallguard":"tower shield","War-pick":"war pick"};
  ...
  function wondrousKind(type,noun){
    if(WOND_FIX[noun]) return WOND_FIX[noun];
    var n=String(noun||"").toLowerCase();
    if(!n){
      var fallback={Helmet:"helm",Cloak:"cloak",Necklace:"amulet",Ring:"ring",Gloves:"gloves",Belt:"belt",Boots:"boots"};
      return fallback[type]||String(type||"item").toLowerCase();
    }
    if(n==="gauntlets"||n==="gloves"||n==="bracers") return n;
    if(n.charAt(n.length-1)==="s") return n;
    return n;
  }
